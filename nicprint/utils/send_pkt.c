...
/* IEEE80211 header */
// I should chnage this to take dest MAC as input
static u8 ieee_hdr_data[] =
{
	...
	0x00, 0xc0, 0xca, 0xb7, 0x95, 0x5a, // Destination
	0x22, 0x23, 0x23, 0x23, 0x23, 0x23, // Transmitter address
...
int main(int argc , char*argv[])
{
	...
	if(argc < 3)
	{
		printf("Usage: %s <RATE> <DST_MAC>\n", argv[0]);
		exit(-3);
	}

	int rate_index = atoi(argv[1]);

	if(rate_index < 0 || rate_index > 7)
	{
		printf("Error: <RATE> must be [0,7]\n");
		exit(-4);
	}

	char * hex_str_mac = argv[2];
	uint8_t mac_dest_vic[6];
	if (sscanf(hex_str_mac, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
               &mac_dest_vic[0], &mac_dest_vic[1], &mac_dest_vic[2],
               &mac_dest_vic[3], &mac_dest_vic[4], &mac_dest_vic[5]) != 6) {
	...
	int i, nLinkEncap = 0, payld_size=69, nDelay = 10000, ieee_hdr_len = sizeof(ieee_hdr_data);
	int radio_tap_hdr_len = sizeof(u8aRadiotapHeader);
	...
	buf_p[15] = (unsigned char)rate_index; // change first byte of source to be the MCS
	for (int i = 0; i < 6; i++) {
		buf_p[dest_strt_idx + i] = mac_dest_vic[i];
	}
	printf("\n");
